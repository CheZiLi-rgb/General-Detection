...
def download_oss_file(local_file_path):
    local_file_path = os.path.abspath(local_file_path)

    if not os.path.exists(local_file_path):
        raise FileNotFoundError(f"文件不存在，请检查路径: {local_file_path}")

    try:
        with open(local_file_path, "rb") as f:
            file_content = f.read()
    except IOError as e:
        raise IOError(f"文件读取失败（权限不足或文件损坏）: {local_file_path}, 错误: {str(e)}")

    img = np.frombuffer(file_content, dtype=np.uint8)
    img = cv2.imdecode(img, flags=cv2.IMREAD_COLOR)

    if img is None:
        raise ValueError(f"图像解码失败，文件可能损坏或格式不支持: {local_file_path}")
...
class Ours_Dataset_train(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Loading index 19459:/data1/lic/DeMamba-main/Preprocess/GenVideo-100K/train_DynamicCrafter/train_DynamicCrafter_frames/DynamicCrafter_6492
        # print("Loading index {}:{}".format(idx, video_id))
        label_onehot = [0]*2
        select_frame_nums = 8
