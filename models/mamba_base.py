...
class ResidualBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.drop_path = DropPath(drop_prob=config.drop_prob) # 用于在训练过程中随机丢弃路径
...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        xz = self.in_proj(x) # (B, L, 2*ED) (784, 8, 2*1536)
        x, z = xz.chunk(2, dim=-1) # (B, L, ED), (B, L, ED) (784, 8, 1536)

        # x branch
        x = x.transpose(1, 2) # (B, ED, L) (784, 1536, 8)
        x = self.conv1d(x)[:, :, :L] # depthwise convolution over time, with a short filter # (784, 1536, 8)
        x = x.transpose(1, 2) # (B, L, ED) (784, 8, 1536)
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        output = x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps) * self.weight # (784, 8, 768)
