...
def train_one_epoch(cfg, model, loss_ce, scheduler, optimizer, epochID, max_epoch, max_acc, train_loader, val_loader, snapshot_path):
    ...
    if (epochID+1) % 1 == 0:
        ...
        # real 
        condition = df_result['data_path'].apply(lambda x: x.startswith("/data1/lic/DeMamba-main/Preprocess/GenVideo-100K/GenVideo-Val/Real/GenVideo-Val-Real"))
...
def train_one_epoch2(cfg, model, loss_ce, optimizer, epochID, max_epoch, max_acc, train_loader, val_loader, snapshot_path):
    model.train()
    trainLoss = 0
    lossTrainNorm = 0
